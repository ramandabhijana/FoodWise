...
//                entryConversations = [newConversation]
  ...
  func createNewConversation(userId: String,
    ...
    Future { [weak self] promise in
      ...
      let recipientConversation = Conversation(id: UUID().uuidString, chatRoomId: chatRoom.id, otherUserId: userId, otherUserType: kCustomerType, latestMessage: latestMessage)
              ...
              self.db.collection(self.chatRoomPath)
                ...
                .getDocument(as: ChatRoom.self) { result in
                  ...
                  case .success(var room):
                    self.sendMessage(message, otherUserType: kCustomerType, toChatRoom: room)
                      .sink { completion in
