...
//        }
    ...
    // Get all sessions, iterate
    // refetch once again to check if it is already assigned
    // if not assigned
    //    -> set and listen for changes
    //    -> maximum rejection = 3
    //    -> each session only has 30sec deadline
    // if assigned -> move to next session on the list
    //
  }
