...
struct WalletDetailsView: View {
  ...
  init() {
    _viewModel = StateObject(wrappedValue: WalletDetailsViewModel())
  }
  ...
  var body: some View {
          ...
          VStack(spacing: 28) {
            ...
            HStack(spacing: 16) {
              Button(action: {
                ...
                let userName = mainViewModel.merchant.name
                let userEmail = mainViewModel.merchant.email
    ...
    .onAppear {
      ...
      viewModel.loadWallet(userId: mainViewModel.merchant.id)
    }
